...
const connectDB = async () => {
  try {
    const conn = await mongoose.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/palvoria-properties', {
    ...
    logger.warn('Database connection failed, running in standalone mode:', error.message)
    logger.info('To enable database features, make sure MongoDB is running on localhost:27017')
    return false
  }
}
...
app.get('/api/health', (req, res) => {
  res.json({
    ...
    websocket: socketService.io ? 'initialized' : 'not_initialized'
  })
})
...
app.use('*', (req, res) => {
  res.status(404).json({
    ...
    method: req.method,
    url: req.originalUrl,
    availableRoutes: ['/api/auth', '/api/properties', '/api/users', '/api/leads', '/api/chat', '/api/search', '/api/analytics', '/api/mortgage', '/api/neighborhoods', '/api/uploads']
  })
})
...
const startServer = async () => {
  try {
    ...
    // Start HTTP server
    server.listen(PORT, () => {
      logger.info(`🚀 Palvoria Properties Backend running in ${process.env.NODE_ENV || 'development'} mode`)
      ...
      console.log(`📅 Database: ${dbConnected ? 'Connected' : 'Standalone Mode'}`)
      console.log(separator)
    })
  } catch (error) {
